...
public class HockeyAppAndroid : MonoBehaviour
{
	...
	protected virtual WWWForm CreateForm (string log)
	{
        ...
        string messageString = "<color=#ffffff>" + GetType().ToString() + " -> CreateForm()\n</color><color=#999999>writenToFile:\n"+writenToForm+"</color>";

        LogEverythingLog.LogMessage(messageString);
	...
	protected virtual IEnumerator SendLogs (List<string> logs)
	{
        ...
        string messageString = "<color=#ff0099>" + GetType().ToString() + " -> SendLogs(), url = '"+url+"', logs.Count = "+logs.Count+"</color>";
        LogEverythingLog.LogMessage(messageString);
		...
		foreach (string log in logs) {		
            ...
            string messageString2 = "<color=#ff00ff>" + GetType().ToString() + " -> SendLogs(), log = '"+log+"'</color>";
            LogEverythingLog.LogMessage(messageString2);
	...
	protected virtual void WriteLogToDisk (string logString, string stackTrace)
	{
        ...
        string messageString = "<color=#ffffff>" + GetType().ToString() + " -> WriteLogToDisk()\n</color><color=#999999>writenToFile:\n"+writenToFile+"</color>";
	...
	protected virtual void HandleException (string logString, string stackTrace)
	{
        ...
        string messageString = "<color=#ffffff>" + GetType().ToString() + " -> HandleException(), logString = '"+logString+"'\n</color><color=#999999>stackTrace:\n"+stackTrace+"</color>";
	...
	{
        string messageString = "<color=#ffffff>" + GetType().ToString() + " -> OnHandleLogCallback(), logString = '"+logString+"', type = '"+type.ToString()+"'\n</color><color=#999999>stackTrace:\n"+stackTrace+"</color>";
	...
	{
        string messageString = "<color=#cccccc>" + GetType().ToString() + " -> OnHandleUnresolvedException(), sender = '" + sender.ToString() + "', args = '" + args.ToString() + "'</color>";
